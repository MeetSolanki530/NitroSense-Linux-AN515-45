/**
 * Exercises DamxClient against the mock daemon. Covers the three things that
 * are hard about this protocol: split replies, one-in-flight serialisation,
 * and disruptive commands that never reply.
 */
import { DamxClient, DamxError } from '../electron/damx-client.ts';

const SOCK = process.env.DAMX_SOCKET ?? '/tmp/damx-mock.sock';
...
async function main(): Promise<void> {
  ...
  console.log('\n3. Oversized request rejected before it hits the wire');
  {
    ...
    check('rejects >4096-byte request', err instanceof DamxError && (err as DamxError).code === 'E2BIG',
      String(err));
    c.close();
  }
  ...
  {
    const c = new DamxClient('/tmp/damx-does-not-exist.sock');
    let err: unknown;
    try { await c.send('get_version'); } catch (e) { err = e; }
    check('ENOENT surfaces an actionable message',
      err instanceof DamxError && (err as DamxError).code === 'ENOENT' &&
