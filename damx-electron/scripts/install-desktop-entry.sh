...
#   ~/.local/share/applications/damx.desktop
#   ~/.local/share/icons/hicolor/<size>/apps/damx.png
#
...
DESKTOP_FILE="$HOME/.local/share/applications/damx.desktop"
...
if [ "${1:-}" = "--uninstall" ]; then
  rm -f "$DESKTOP_FILE"
  for s in $SIZES; do rm -f "$ICON_ROOT/${s}x${s}/apps/damx.png"; done
  refresh_caches
...
for s in $SIZES; do
  ...
  [ -f "$src" ] || continue
  install -Dm644 "$src" "$ICON_ROOT/${s}x${s}/apps/damx.png"
done
...
dim "  icons: $ICON_ROOT/<size>/apps/damx.png"
echo ""
dim "It should appear in the app menu as \"Div Acer Manager Max\"."
