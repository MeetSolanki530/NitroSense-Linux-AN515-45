/**
 * Step-1 deliverable: connect to the real DAMX daemon and report exactly what
 ...
 */
import { DamxClient, DamxError, DEFAULT_SOCKET_PATH } from '../electron/damx-client.ts';
...
async function main(): Promise<void> {
  ...
  console.log(`\n${bold('DAMX daemon probe')}  ${dim(client.socketPath)}\n`);

  try {
    await client.connect();
    console.log(green('  Connected.') + '\n');
  } catch (e) {
    const err = e as DamxError;
    console.log(red('  Could not connect.'));
    console.log(`  ${err.message}\n`);
    console.log(dim('  Checks:'));
    console.log(dim('    systemctl status damx-daemon.service'));
