/**
 * Internals Manager — driver/model management on top of HardwareClient.
 *
 ...
 * (all verified against service/nitrosense-daemon.py):
 *
 ...
 * through HardwareClient's disruptive path: send, tolerate the dropped
 * connection, reconnect, then re-read state to find out what really happened.
 */

import { HardwareClient, DISRUPTIVE_COMMANDS } from './hardware-client.ts';
...
export class InternalsManager {
  ...
  constructor(client: HardwareClient) {
    this.#client = client;
  }
  ...
  async #runDisruptive(command: string): Promise<OperationResult> {
    ...
    try {
      ...
      // A dropped connection is the expected outcome here; HardwareClient already
    ...
    if (!daemonReturned) {
      return {
        ...
        warning:
          ...
          'sudo systemctl restart nitrosense-daemon.service',
      };
    }
