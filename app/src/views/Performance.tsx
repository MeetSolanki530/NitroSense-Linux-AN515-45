...
import type { ConnectionState, PowerMode, PowerState, Settings, Telemetry } from '../state/hardware';
...
export function Performance({
  ...
  const pushFan = useDebounced((cpu: number, gpu: number) => {
    ...
    void run(() => window.nitrosense.setFanSpeed(cpu, gpu)).then(() => {
      cpuFan.reset();
      gpuFan.reset();
    });
  }, 150);
  ...
  const setAuto = (): void => {
    ...
    cpuFan.setPending(0);
    gpuFan.setPending(0);
    void run(() => window.nitrosense.setFanSpeed(0, 0)).then(() => {
      cpuFan.reset();
      gpuFan.reset();
    });
  };
  ...
  const selectPowerMode = (mode: PowerMode): void => {
    ...
    void runMode(() => window.nitrosense.setPowerMode(mode)).then(() => {
      currentMode.reset();
      cpuFan.reset();
      gpuFan.reset();
    });
