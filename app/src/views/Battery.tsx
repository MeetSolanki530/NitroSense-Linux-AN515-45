...
import type { ConnectionState, Settings, Telemetry } from '../state/hardware';
import './Battery.css';
    ...
    <div className="battery-view">
      ...
      >
        <Toggle
          ...
          onChange={(next) => void run(() => window.nitrosense.setBatteryLimiter(next))}
        />
      </ControlBlock>
      ...
      >
        <Toggle
          ...
          onChange={(next) => {
            void run(() => window.nitrosense.setBatteryCalibration(next));
        ...
        <div className="usb-levels">
          {USB_LEVELS.map((level) => (
            <button
              ...
              onClick={() => void run(() => window.nitrosense.setUsbCharging(level as UsbLevel))}
            >
              {usbLabel(level)}
            </button>
          ))}
      ...
      <ControlBlock title="Display & Boot" gate={connectionGate(connected)}>
        ...
        <Toggle
          ...
          onChange={(next) => void run(() => window.nitrosense.setBootAnimationSound(next))}
        />
        <Toggle
          ...
          onChange={(next) => void run(() => window.nitrosense.setBacklightTimeout(next))}
        />
