...
import type { NitroKeyState } from '../state/hardware';
...
export function NitroKeySetup(): JSX.Element | null {
  ...
  useEffect(() => {
    void window.nitrosense.nitroKeyState().then(setState).catch(() => undefined);
  }, []);
  ...
  useEffect(() => {
    return window.nitrosense.onNitroKey(({ accelerator }) => {
      setPhase((p) => {
        if (p !== 'listening') return p;
        setBound(accelerator);
        void window.nitrosense
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      void window.nitrosense.nitroKeyCancel().catch(() => undefined);
      setPhase('notfound');
    }, LISTEN_TIMEOUT_MS);
  ...
  const begin = (): void => {
    setPhase('listening');
    void window.nitrosense.nitroKeyBegin().then(({ ok }) => {
  ...
  const decline = (): void => {
    void window.nitrosense.nitroKeyDecline().catch(() => undefined);
