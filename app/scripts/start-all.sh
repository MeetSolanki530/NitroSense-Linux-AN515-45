...
cd "$(dirname "$0")/.."
PROJECT="$PWD"
DAEMON_SRC="$PROJECT/../service/nitrosense-daemon.py"
SOCK="/var/run/nitrosense.sock"
...
PIDFILE="/tmp/nitrosense-daemon.pid"
...
EXISTING="$(pgrep -f 'nitrosense-daemon\.py' || true)"
...
if [ -n "$EXISTING" ] && [ ! -S "$SOCK" ]; then
  ...
  if [ ! -S "$SOCK" ]; then
    warn "  Found a service process but no socket — it is not serving."
    dim "  Clearing it so a working one can start."
    sudo pkill -f 'nitrosense-daemon\.py' 2>/dev/null || true
    sleep 1
    EXISTING=""
  fi
fi
...
if [ -n "$EXISTING" ]; then
  ...
  dim "  To use this script's own service instead:  sudo pkill -f nitrosense-daemon.py"
else
